...
\parinventory
& \idx{|glstyle|}& choice (|wrap|, |multilevel|, |3level|, or |oldstyle|)&
|wrap|\cr
\endparinventory
...
|glstyle|, which can be set to |wrap|, |multilevel|, |3level|, or
|oldstyle|.  The last is included for backwards compatibility
with the early version of ExPex.  It is not intended for current
or future use.
...
Multilevel glosses have a drawback, but also a distinct advantage.
The drawback is that they do not wrap automatically. The major
advantage is that |\gla| and |\glb| can be used multiple times,
as shown below.  Secondarily, it is easy to give citations inline
with the gloss.
...
\codedisplay
\ex[lingstyle=glabc,glstyle=multilevel]
\begingl
\gla k- wapm -a -s'i -m -wapunin -uk //
\glb Cl V Agr Neg Agr Tns Agr //
