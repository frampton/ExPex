...
which repeats \getref{basicglosssec}.
...
One big advantage to this style of coding complex glosses is that the
gloss of a word and the word are adjacent in the code, as they are in
the display which is produced; vertically adjacent in the display,
horizontally adjacent in the code. This makes the code much more
readable. In effect, an aspect of WYSIWG is built into the coding.
This is particularly useful if the gloss has many words, the gloss of
a narrative for example.
...
only for clarity.  The code below is equivalent.
...
Instead, the lines must be referred to positionally.
...
constructed.  An adjustment is made accordingly.  In nlevel style
coding, the first glword is boxed and contributed to a paragraph
before the material to construct the second glword is read, so there
is no way for any adjustment to be made automatically.
