...
%   glneveryline={\hfil\eightrm\insertno,\it},
%   glnabovelineskip={,-1pt},glspace=!.4em]
%\count0=1
%\def\insertno{(\the\count0)\global\advance\count0 by 1}%
%\begingl
%{}[Hom\^{a}o/\textsc{exist}]
%[sa/one]
%[\v{c}\^{o}/\textsc{clf}]
...
\section Glosses; Special topics
...
\deftag{\the\secno.\the\subsecno}{cascadingsec}
...
underfixes are positioned somewhat differently.
...
left aligned.  Occasionally it is desirable, for one reason or
another, to center align a particular line by inserting an |\hfil| on
the left, to balance the implicit |\hfil| on the right.  Compare, for
example, (\nextx) with (\getref{wapm2}).

\framedisplay
\ex[glspace=1.5em,everygla=\hfil,everyglb=\hfil\footnotesize,
...
Center alignment in glwords is particularly useful if you need to
number the glwords; to facilitate discussion, for example.
...
\codedisplay~
\ex[glstyle=nlevel,glhangstyle=cascade,
   glneveryline={\hfil\eightrm,\it,},glnabovelineskip={,-1pt},
   glspace=!.4em]
\begingl
...
\medskip
\noindent
Note that a line 1 entry is made for the first glword.  \ExPex\
tolerates missing entries, but at least one glword must have an entry.
|\relax| is sufficient.
