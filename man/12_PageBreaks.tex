...
also vary between different stages of production. There is no
sense in spending time on formatting while you are still working
on a draft.  \expex\ therefore provides a number of parameters
which can be adjusted to control various aspects of page
breaking.
...
usually be seen when editing.  The danger is that either the
problem is missed, or the output is not obviously bad, but less
than ideal.
...
Macro:\quad |\exbreak|
\parinventory
& \idx{|exbreakfil|}& skip& |0pt plus 4ex|\cr
...
block.  The penalty and amount of fill are parametrized.  The
...
keeps most examples intact at the cost of allowing substantial
whitespace to appear at the bottom of the page.
...
In the beta version of \Expex, |\exbreak| was called |\goodpar|.
...
%\subsection  User defined choice of formatting between different
%kinds of documents
%
%Different kinds of documents call for different settings of the
%basic parameters, \Expex\/ parameters and basic \Tex\/ or
%\Latex\/ parameters.
%
%\codefragment
%|\def\draft{%|
%\quad |\lingset{|\dots|}|\par
%\quad {\sl set the headline to give the date on each page}\par
%\quad {\sl set the baseline to twice the normal baseline}\par
%\quad |\rightskip=0pt plus 3em|\par
%\quad |\raggedbottom|
%
%
%
