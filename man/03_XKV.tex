...
exagerated spacing when an example directly follows another one, with
no intervening text.
...
on an entirely parallel manner.

\smallskip
\noindent {\it Choice parameter\/}:\enspace The {\sl value\/} which is
...
Frequent reference is made to the ``value of a key'', meaning the
result of expanding the macro associated with the key in case the key
is a command key or incremental key (either a dimension or skip).  The
notation {\tt \char"7C{\sl key}\char"7C} is often used to indicate the
value of the key.  So, for example, \parvalue{textoffset} is the value
associated with the key |textoffset|.
