...
Macros:\quad |\tspace[]|, |\exnoprint|, |\crnb|
\bigskip

\noindent Most of the difficulty of formatting example displays
...
\Tex\/ primitive |\halign|.

Many tabular examples have the form

\codefragment
|\ex \vtop{\halign{|\quad\dots\quad |}}\xe|
\endcodefragment
...
Macros:\quad  |\labels[]|, |\tl|, |\nl|
\bigskip

\noindent If reference must be made to particular lines in
(\lastx), the lines need labels of some sort.  One approach is to
explicitly enter the line labels a--e:
...
We will see in Section \getref{named-reference} that lines in
tables can be named and reference made by name, using
implicit line numbering.
...
read an optional tag using the |<|\dots|>| mechanism.
...
Macros: \idx{|\lingdima|}, \idx{|\lingdimb|},
   \idx{|\lingdimc|}, \idx{|\tspace|}|[]|
\parinventory
& \idx{|dima|}& dimension& |2.4em|\cr
& \idx{|dimb|}& dimension&\cr
& \idx{|dimc|}& dimension&\cr
& \idx{|crskip|}& skip& |1.2ex|\cr
\endparinventory

\noindent Tables often need considerable adjustment in order to
...
Macros:\quad |\exdisplay|, |\noexno|, |\exnoprint|, |\crnb|
\par\nobreak\bigskip

\noindent Up to this point, only tables that are typeset in a
vbox have been considered.  The \Tex\/ page breaking algorithm
does not split a box between pages.  Sometimes, an especially
tall table is needed and one has the choice of floating the table
to the top of the next page (using \Tex's |\topinsert|) or
constructing a breakable table.  The latter choice is often
preferable and can be implemented using \Tex's primitive
|\halign|.

The example number must be part of the |\halign|, not inserted by
|\ex|.  \idx{|\exdisplay|} is designed to accommodate an unboxed
...
cancelled, |\ling@everyex| and |\ling@Everyex| are executed, and
|\excnt| is advanced.
