...
\idx{|labelwidth|}\public & \inc dimension& \textdim{.72 em}\cr
\idx{|preambleoffset|}\public & \inc dimension& \textdim{1 em}\cr
\idx{|interpartskip|}\public & \inc skip&
   \textdim{1 ex} plus \textdim{.2 ex} minus \textdim{.2 ex}\cr
\idx{|belowpreambleskip|}\public & \inc skip&
   \textdim{1 ex} plus \textdim{.2 ex} minus \textdim{.2 ex}\cr
...
below, assuming the default settings of the parameters |labelanchor|
and |textanchor|.  (The effects of changing the settings of the
anchoring parameters will be considered in Section \getref{anchors}.)
Note that the parameters |numoffset| and |textoffset| are also used in
formatting examples without parts.
...
\begininventory
\parameter& \it default value\cr
|nopreamble|& boolean& (not relevant)& \hfil true\cr
\endinventory
...
label, ignoring automatic label generation.  This can be useful if
only some parts of a multipart example are being repeated.  See for
example (\getref{explicit}) in Section~\getref{explicit-sec}.
...
\noindent |label| is the only key that |\a| recognizes.
