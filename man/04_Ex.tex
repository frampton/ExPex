%\lingset{numoffset=2em,textoffset=1ex,aboveexskip=1ex,belowexskip=1ex}
%\ex[aboveexskip=1ex,belowexskip=1ex]
%I wonder which article John filed {\sl t\/} without reading {\sl e}.
%\xe
%\endinput
%\ex[numoffset=2em,textoffset=1ex,aboveexskip=1ex,belowexskip=1ex]
%I wonder which article John filed {\sl t\/} without reading {\sl e}.
%\xe
%\endinput
%\makeatletter
...
Macros:\quad \idx{|\ex|}|~[]|, \idx{|\xe|}, \idx{|\lingnumoffset|},
   \idx{|\lingtextoffset|}\hidewidth\par
\noindent Counter:\quad \idx{|\excnt|}\par
\parinventory
...
& \idx{|aboveexskip|}& skip& 2.7ex plus .4ex minus .4ex\cr
& \idx{|belowexskip|}& skip& 2.7ex plus .4ex minus .4ex\cr
\endparinventory

\noindent
The initial settings of parameters will be given in the section
inventories, if they are relevant.  Pseudo parameters are set for
their immediate effect, not to store a setting associated with
the parameter, so the initial setting of pseudo parameters is not
relevant. |\ex~[]| above should be taken to mean that the macro
...
arguments are permitted.  If the value of a parameter is
accessible by an external macro (i.e. of the form |\ling«key»|),
the macro which expands to the value appears on the list of
macros.

|\xe| terminates constructions initiated by |\ex|.  The following
sample paragraph illustrates the use of |\ex| \dots|\xe|. The
convention in this guide is that text, {\it as it would appear in
...
|\excnt|.  It is incremented when |\ex| is evaluated, before the
number is typeset.  Vertical skip is inserted before and after
...
modification of |\ex| can be used with parameters, so
|\ex~[|\dots|]| will be interpreted correctly.
...
& \idx{|exnoformat|}& \dots |X|\dots& command\cr
\endparinventory

\noindent Suppose that you want to repeat an example that was
given earlier in your document.  Something like.
...
\noindent The regular sequence of example numbering is unaffected
by an example whose number is assigned by |exno|.

|exno| does not have to be set to an integer.
