...
\define@choicekey{ling}{labelalign}[\ling@labelalign\nr]%
   {left,center,right}{%
      \ifcase\nr
            \def\epx@labelprint{\ep@labelformat\epx@label\hss}%
         \or
            \def\epx@labelprint{\hss \ep@labelformat\epx@label\hss}%
         \or
            \def\epx@labelprint{\hss \ep@labelformat\epx@label}%
         \fi
}
...
\ifx\label\relax \else    % else = LaTex is loaded
   \def\latex@tagexlabel{\def\@currentlabel
      {\ep@fullrefformat{{\the\excnt}}{\epx@label}}}%
...
\def\deftaglabel#1{%
   \expandafter\xdef\csname lingtag@\@localextag.#1\endcsname%
      {{{\epx@label}}%
       {{\ep@fullrefformat{\@actualexno}\epx@label}}%
      }%
   \if@g@thertags
      \immediate\write@tags{%
         \noexpand\@fd@f
...
%      {\ep@fullrefformat{\@actualexno}\epx@expandonce\epx@label}%
%      }%
%   \ignorespaces
%}
