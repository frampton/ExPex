...
parameters are discussed, here are a few examples done with the
...
typeset.  These vboxes, outlined in (\lastx), will be called glwords.
A strut is inserted on each line of each glword.\footnote{It is
possible to turn off strut insertion inside glwords by setting the
boolean parameter |glstruts| to |false|.  It is unlikely that you will
use this feature, except perhaps for diagnosing a spacing problem.  It
was introduced as an aid in designing and building the \ExPex\
glossing machinary.  It remains because it might be useful to someone
at some point.} As these boxes are generated, they are fed to \Tex's
standard paragraph building machinery, with \parvalue{glspace} the
interword space.
\smallskip
...
\framedisplay
\ex[glspace=1.5em,everygla=,everyglb=\footnotesize,aboveglbskip=-.2ex]<wapm2>
\begingl
\gla k- wapm -a -s'i -m -wapunin -uk //
\glb CL V AGR NEG AGR TNS AGR //
...
to |\it|.  There are a number of hooks
of this type.
...
vertical spacing.  They are all \inc skip parameters.
...
pt}, but a setting of \textdim{.5 ex} or \textdim{1 ex} makes the
...
overfull lines.  The choice parameter |glhangstyle| specifies the
kind of hanging indentation which is used in the interlinear gloss.
...
The examples below are set with {\it normal\/} hanging indentation
(the initial setting), and {\it no\/} hanging indentation, and {\it
cascading\/} hanging indentation.  The hanging indentation is
\textdim{1 em} (the initial setting).
...
Consider (\getref{wapm2}) again.  The code can be written as
...
\noindent  Above, the scope of the parameters is limited to the line
macros they modify.  This is conceptually satisfying, but it is more
convenient to specify them all in one place.
...
easier to see what the parameter setting are.
...
characters will disappear from \ExPex\ at some point.  |\nogloss|
...
Below, the space between the prefix and the verb has been omitted.
...
\section An alternate syntax for coding glosses
