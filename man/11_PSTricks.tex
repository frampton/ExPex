...
Several features of {\sl ExPex\/}, listed in (\nextx), come into
...
used to set the {\sl PSTtricks\/} parameters |angle|, |arrows|,
|nodesep|, |labelsep|, and |linearc|, and the \Expex\/ parameter
|dima|.  Note also that the {\sl ExPex\/} scratch dimension
...
\noindent Since the connections which {\sl PSTricks\/} draws are
...
\Pstricks\/ is active.  If a key
name in the family {\sl ling\/} is the same as a key name in
another family which has also been added as part of the {\sl
PSTricks\/} extended family, |\Lingset| will treat it
unambiguously as in the {\sl ling\/} family.   |\psset|, on the
other hand, does not give priority to keys in the {\sl ling\/}
family.  Exactly how |\psset| establishes priority is a
complex matter, depending on the history of the formation of the
{\sl PSTricks\/} extended family.

\endinput
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Below, a complex display appears in a labeled part of a |\pex|
construction.

\pex[interpartskip=1em,nopreamble]
\def\\#1(#2){\rnode{#2}{\strut #1}}%
\def\[#1{$[_{\rm #1}$}%
\def\,{\hskip.25ex}%
%
\a
\[{TP} Juan se lo [\,[ mostr\'o[$v$]\,]
\[{VP} t
\[{XP} \dots t\,\dots]\,[\,t t\,]\,]\,]\,]
%
\a \vrule height2em depth3em width0pt
\[{TP} \\Juan(J) \\se(se) \\lo(lo) [\,[ \\mostr\'o(V)[$v$]\,]
\[{VP} \\t(t1)
\[{XP} \dots \\t(t2)\,\dots]\,[\,\\t(t3) \\t(t4)\,]\,]\,]\,]
\psset{angle=90,nodesep=0pt,armA=1em,arrows=-}
\ncbar{t2}{J}
\ncbar{t1}{se}
\ncbar{t4}{t2}
\mput*{$\times$}
\psset{angle=-90,arrows=->}
\ncbar{t1}{J}
\ncbar[armA=1.5em]{t2}{se}
\ncbar[armA=2em]{t4}{lo}
\ncbar[armA=2.5em]{t3}{V}
\xe
