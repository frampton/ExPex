...
\def\begininventory@a{\medskip\hrule\medskip \bgroup
   \openup2pt
   \halign\bgroup \quad ##\hfil&&
      \hskip \ifx\@optionalarg\empty 2em \else\@optionalarg\fi ##\hfil\cr
}
\def\endinventory{\egroup\egroup \medskip\hrule\bigskip\noindent}
...
\def\macros@a{Macro\if@star\else s\fi:\hidewidth\cr}
\def\counter{\omit Counter:\hidewidth\cr}
\def\endmacros{\hidewidth\cr}
...
\def\textdimA#1 #2 {#1\hskip.12em #2}
\def\suff#1{{-\small #1}}%
\def\small{\tenrm}
