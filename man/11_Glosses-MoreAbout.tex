...
\subsection The parameters {\tt glossbreaking} and {\tt glwidth}

\begininventory
\parameters*
\idx{|glwidth|}& dimension& \textdim{0 pt}\cr
\idx{|glbreaking|}& boolean& true\cr
\endinventory
...
If |glwidth| is set to \textdim{0 pt} the gloss is constructed in a vbox only if
|glossbreaking| is set to |false|.  This is the initial setting.\footnote{The
initial setting is |false| for reasons of backwards compatability.} The width of
the  box is The width is $h-l-r$, where $h$, $l$, and $r$, are the current
values of |\hsize|, |\leftskip|, and |\rightskip|, respectively.  This implicit
determination of the width of the gloss is appropriate for use with the \ExPex\
macros which typeset examples.  These macros adjust the leftskip, so the width
of the gloss vbox must be adjusted to correspond to the available space.

If |glwidth| is set to \textdim{0 pt} and |glossbreaking| is true, the gloss is
not constructed in a vbox, so it can {\it break between pages}.  This is
particularly useful in glossing stories or translations, which frequently run
over several pages.

Note that although the initial setting of |glossbreaking| is |false|, the
default setting is |true|.  This means that |\lingset{glossbreaking}| is
equivalent to |\lingset{glossbreaking=true}|. Parameters that are set with no
value specified are set to their default values.
...
%if the parameter |glwidth| is set to \textdim{0 pt}.
%
