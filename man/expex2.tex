...
%% from multiwrap5
\define@choicekey{ling}{glstyle}[\ling@glstyle\nr]%
   {3level,wrap,oldstyle,multilevel,multiwrap}{%
      \ifcase\nr
            \let\begingl@type=\begingl@T
            \let\endgl\endgl@T
            \glmw@letlevels
            \let\glft=\glmw@glft
         \or
            \let\begingl@type\glmw@begingl@aa
            \let\endgl\glmw@endgl
            \glmw@letlevels
            \let\glft=\glmw@glft
         \or
            \let\begingl@type=\begingl@L
...
%% \glmw@levels is the list of mw-levels that have been defined
\def\glmw@letlevels{%
   \expandafter\XKV@for@n\expandafter{\glmw@levels}\levelname
   {\edef\temp{\noexpand\let
   \expandafter\noexpand\csname gl\levelname\endcsname
   \expandafter\noexpand\csname glmw@gl\levelname\endcsname}%
   \temp}%
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\def\lop#1\to#2{\ifx#1\empty
   \let#2\empty \else\expandafter\lopoff#1\lopoff#1#2\fi}
\long\def\lopoff\\#1#2\lopoff#3#4{\def#4{#1}\def#3{#2}}
\newif\ifgl@loopmore
\def\glmw@mergerow#1\to #2{%
   \let\alist=#1%
   \let\blist=#2%
   \let\clist=\empty
   \gl@loopmoretrue
   \loop \ifgl@loopmore\glmw@mergerow@a \repeat
   \global\let#2=\clist
}
\def\glmw@mergerow@a{%
   \lop\blist\to\tempb
   \ifx\tempb\Linebreak
         \expandafter\gappend\Linebreak\to\clist
      \else\ifx\tempb\Closeup
         \expandafter\gappend\Closeup\to\clist
      \else
         \lop\alist\to\tempa
         \expandafter\gappend \tempa \to \tempb
         \expandafter\gappend \tempb \to \clist
      \fi\fi
...
\def\glmw@gla@a #1// {\bgroup\ling@usearg
   \glmw@gla@b #1 \@nil }
\def\glmw@gla@b{\@ifnextchar\@nil\glmw@gla@c\glmw@gla@d}
\def\glmw@gla@c#1{\glmw@updateauxlistsA \egroup}
\def\glmw@gla@d #1 {%
   \def\tempa{#1}\let\tempb=\tempa
   \@onelevel@sanitize\tempa
   \setbox0=\hbox{\ling@everygla #1}%
   \ifdim\glmw@maxht<\ht0 \glmw@maxht=\ht0 \fi
   \ifdim\glmw@maxdp<\dp0 \glmw@maxdp=\dp0 \fi
   \ifx\tempa\Linebreak
         \expandafter\gappend\Linebreak\to\mainlist
      \else\ifx\tempa\Closeup
         \expandafter\gappend\Closeup\to\mainlist
      \else
         \edef\temp{\noexpand\\{\expandonce\ling@everygla\noexpand\strut
         \expandonce\tempb}}%
         \expandafter\gappend\temp\to\mainlist
      \fi\fi
   \glmw@gla@b
}
\def\glmw@updateaux@a{%
   \expandafter\gappend\temp \to\aboveskiplist
   \edef\temp{\vrule width0pt height\the\glmw@maxht\space depth\the\glmw@maxdp\space }%
   \expandafter\gappend\temp\to\strutlist
%   \expandafter\gappend\gl@linelabel\to\labellist
}
\def\glmw@updateauxlists{%
   \edef\temp{\vskip\csname ling@abovegl\gl@linelabel skip\endcsname}%
   \glmw@updateaux@a
}
\def\glmw@updateauxlistsA{%
   \let\temp=\empty
   \glmw@updateaux@a
}
\def\glmw@levels{a,ft}
\def\glmw@mklevel#1{%
   \expandafter\ifx\csname glmw@gl#1\endcsname\relax
      \define@ling@cmdkeys{everygl#1}%
      \define@linginckey\skip{}{@}{abovegl#1skip}%
      \XKV@addtolist@o\glmw@levels{#1}%
      \lingset{everygl#1=,abovegl#1skip=0pt}
      \expandafter\def\csname glmw@gl#1\endcsname{\@glmw{#1}}%
   \else
...
\def\definemwlevels#1{\XKV@for@n{#1}\thislevel
   {\expandafter\glmw@mklevel\expandafter{\thislevel}}}
\def\@glmw#1{%
   \def\worklist{}%
   \glmw@maxht=0pt
   \glmw@maxdp=0pt
   \def\gl@linelabel{#1}%
   \@getoptionalarg\@glmw@a
}
\def\@glmw@a #1// {\bgroup
   \ling@usearg
   \@glmw@b #1 \@nil }
\def\@glmw@b{\@ifnextchar\@nil\@glmw@c\@glmw@d}
\def\@glmw@c#1{\glmw@updateauxlists \glmw@mergerow\worklist\to\mainlist\egroup}
\def\@glmw@d #1 {%
   \def\tempa{#1}%
   \edef\temp{\expandafter\expandonce\csname ling@everygl\gl@linelabel\endcsname
      \noexpand\strut \expandonce\tempa}%
   \setbox0=\hbox{\temp}%
   \ifdim\glmw@maxht<\ht0 \glmw@maxht=\ht0 \fi
   \ifdim\glmw@maxdp<\dp0 \glmw@maxdp=\dp0 \fi
   \expandafter\gappend\temp\to\worklist
   \@glmw@b
}
\newif\ifgl@spacebefore
%%%%%%%%%%%%%%%%
\def\glmw@endgl{\glmw@endglA \glmw@endglB}
\def\glmw@endglA{\lineskip=\ling@abovemoreglskip
   \glmw@print \par}
\def\glmw@endglB{\egroup\egroup}
\def\glmw@endgl@a{\lineskip=\ling@abovemoreglskip\glmw@print \par}
%%
%\def\glmw@begingl{\bgroup\@getoptionalarg\glmw@begingl@a}
%\def\glmw@begingl@a{\ling@usearg\glmw@begingl@aa}
\def\glmw@begingl@aa{%
   \parindent=0pt
   \W@prefixboxfalse
   \@ifnextchar\gla\glmw@begingl@@\glmw@begingl@prefix
}
%\def\begingl{\bgroup\@getoptionalarg\@begingl}
%\def\@begingl{\ling@usearg\begingl@type}
\def\glmw@glft#1//{%
...
\def\glmw@print{%
   ...
   \gl@loopmoretrue
   \loop\ifgl@loopmore
      \lop\mainlist\to\tempa\relax       % \tempa is one column
      \ifx\tempa\Linebreak%
         ...
         \else\ifx\tempa\Closeup%
            \gl@spacebeforefalse
         \else
            \ifgl@spacebefore \hskip\ling@glspace \else \gl@spacebeforetrue \fi
            \glmw@printcol
         \fi\fi
