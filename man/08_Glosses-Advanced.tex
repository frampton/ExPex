...
Macro: |\definegllevels|
\medskip

\noindent |\glb| and |\glc| are given definitions in {\it expex.tex\/}
by the command |\definegllevels{b,c}|.  The command also creates the
...
to use |\definegllevels| to create and name new gloss levels.

For example, suppose more suggestive level names are desired.

\codedisplay
\definegllevels{cat,gloss}
...
\framedisplay
\ex[glstyle=wrap,glhangstyle=normal,glufcloseup=.4ex,everygluf=\tenrm]
\begingl
...
{\it ExPex\/} provides the macro |\gluf| which can be used to
...
the tags. The value of |glufcloseup| determines how much the
...
positions the underfix, centered, below the word it annotates.  Its
syntax should be clear from the example above.
...
\noindent This section will not be of any use to most {\it ExPex\/}
