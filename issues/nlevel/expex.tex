...
\def\ExPexMessage
   {2015/01/06 v5.0d ExPex linguistics example formatter (JF)}
% -------------------------------------------------------
% jf 2011/12/09  (v4.0a)
...
\def\exdisplay@b{\let\@optionaltag=\empty \ex@setup \ignorespaces}
%------ \pex -----
\newcount\pexcnt
...
\define@choicekey{ling}{glstyle}[\ling@glstyle\gl@style@num]{wrap,nlevel}[wrap]%
   {\ifnum\gl@style@num=0 \gl@wrap@assignlevels \fi \gl@setprefix }
\define@choicekey{ling}{glftpos}[\ling@glftpos\gl@ftpos@num]{below,right}[below]%
   {\gl@setprefix}
% \gl@prefix is set to one of gl@wrap@below@, gl@wrap@right@, gl@nlevel@below@,
%  and gl@nlevel@right@
\def\gl@setprefix{\edef\gl@prefix{gl@\ling@glstyle @\ling@glftpos @}}
\def\glft{\csname \gl@prefix ft\endcsname}
\def\gl@begingl@a{\csname \gl@prefix begin\endcsname}
\def\endgl{\csname \gl@prefix end\endcsname}
\def\glpreamble{\csname \gl@prefix preamble\endcsname}
% 14 different functions must be defined, 4 (or 2) for each of the 4 prefixes
% <prefix> + begin, preamble, ft, or end  with no preamble for glftpos=right
%%%%%%%%%%%%%%%%%%%%
% 1. the four <prefix> + begin commands
...
\def\gl@nlevel@below@begin@a{%
   ...
   \else
      \vtop\bgroup
         ...
         \leftskip=0pt
   \fi
   \futurelet\temp\gl@nlevel@below@begin@b
}
...
\def\gl@nlevel@right@begin{%
   ...
   \vtop\bgroup
      ...
      \hsize=\ssleftwd \leftskip=0pt
      \hangindent=\ling@glhangindent
      \hangafter=1
      \let\gl@nlevel@below@ft=\gl@nlevel@right@medial
      \begingl
}
%%%%%%%%%%%%%%%%%%%%
% 2. the two <prefix> + preamble commands
\long\def\gl@wrap@below@preamble #1// {%
   \@glpreambletrue
   {\hangindent=0pt \hangafter=0 \ling@everyglpreamble #1\strut
   \par
   \vskip\lingbelowglpreambleskip \leavevmode}%
}
...
%%%%%%%%%%%%%%%%%%%%
% 4. the four <prefix> + end commands
...
%%%%%%%%%%%%%%%%%%%%
% \gl@nlevel@right@begin calls \gl@nlevel@right@medial
...
%%%%%  wrap style parsing, done by \gla, \glb, etc.
% \glw@ is used as a prefix for all of the internal commands
% various lists are created as the parsing proceeds
% \mainlist, \itemtypelist, \aboveskiplist, \strutlist, \everylist
% mainlist is a list of lists of items, the first item in mainlist
% is the list of items that are stacked up in the first vbox which
% is shipped to Tex's line making machinary
% needs much more documentation
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% introduced "glossbreaking" boolean parameter
\define@boolkey{ling}[ling@]{glossbreaking}[true]{}
...
\let\ling@glftpos\relax  % a trick to allow forward reference
\lingset{glstyle=wrap,glftpos=below}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% PRINTING FROM THE LISTS BUILT BY THE WRAP PARSING MACHINERY
% Commands which read the lists and produce the boxes which are fed into Tex's
% line building machinary.
...
%%%%%%%%%%%%%%%%%%%% NLEVEL PARSING AND BOX ASSEMBLY %%%%%%%%%%%%%%%%%%%%
%% \gln@ is used as a prefix for the internal commands
\def\ep@setglstrut{%
...
% dimension used in wrap@right and nlevel@right constructions
