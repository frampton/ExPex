...
\define@choicekey{ling}{glstyle}[\ling@glstyle\gl@stylenum]%
   {wrap,3level,oldstyle,multilevel,multiwrap}{%
      \ifcase\gl@stylenum
            \let\begingl@type\glw@begingl@aa
            \let\endgl\glw@endgl
            \glw@letlevels
            \let\glft=\glw@glft
         \or
...
\newif\ifgl@loopmore
\def\gl@mergerow#1\to #2{%
   \let\alist=#1%
   \let\blist=#2%
   \let\clist=\empty
   \gl@loopmoretrue
   \loop \ifgl@loopmore\gl@mergerow@a \repeat
   \global\let#2=\clist
}
\def\gl@mergerow@a{%
   \gl@lop\blist\to\tempb
   \ifx\tempb\Linebreak
         \expandafter\gl@append\Linebreak\to\clist
      \else\ifx\tempb\Closeup
         \expandafter\gl@append\Closeup\to\clist
      \else\ifx\tempb\Lbrack
         \expandafter\gl@append\Lbrack\to\clist
      \else\ifx\tempb\Rbrack
         \expandafter\gl@append\Rbrack\to\clist
      \else
         \gl@lop\alist\to\tempa
...
\def\glw@gla@d #1 {%
   ...
   \ifdim\gl@maxht<\ht0 \gl@maxht=\ht0 \fi
   \ifdim\gl@maxdp<\dp0 \gl@maxdp=\dp0 \fi
   \ifx\tempa\Linebreak
         \expandafter\gl@append\Linebreak\to\mainlist
      \else\ifx\tempa\Closeup
         \expandafter\gl@append\Closeup\to\mainlist
      \else\ifx\tempa\Lbrack
         \expandafter\gl@append\Lbrack\to\mainlist
      \else\ifx\tempa\Rbrack
         \expandafter\gl@append\Rbrack\to\mainlist
      \else
         \edef\temp{\noexpand\\{\expandonce\ling@everygla
            \expandonce\tempb}}%
         \expandafter\gl@append\temp\to\mainlist
      \fi\fi\fi\fi
   \glw@gla@b
}
\def\glw@updateaux@a{%
   \expandafter\gl@append\temp \to\aboveskiplist
   \edef\temp{\vrule width0pt height\the\gl@maxht\space depth\the\gl@maxdp\space }%
   \expandafter\gl@append\temp\to\strutlist
%   \expandafter\gl@append\gl@linelabel\to\labellist
}
...
\def\@glmw@c#1{\glw@updateauxlists \gl@mergerow\worklist\to\mainlist\egroup}
\def\@glmw@d #1 {%
   \def\tempa{#1}%
   \edef\temp{%
   ...
   \ifdim\gl@maxht<\ht0 \gl@maxht=\ht0 \fi
   \ifdim\gl@maxdp<\dp0 \gl@maxdp=\dp0 \fi
   \expandafter\gl@append\temp\to\worklist
   \@glmw@b
}
\definemwlevels{b}
