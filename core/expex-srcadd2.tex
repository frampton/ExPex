...
% building machinary.
%
% \ep@samecharcode is intended to be used in the context \if ___
% the arguments must expand to tokens
...
\def\glw@mergerow#1\to #2{%
   ...
   \loop
      \ifgl@loopmore
         \gl@lop\blist\to\tempb
         \if\ep@samecharcode\tempb +\gl@append +\to\clist
            \else\if\ep@samecharcode\tempb @\gl@append @\to\clist
            \else\if\ep@samecharcode\tempb [\gl@append [\to\clist
            \else\if\ep@samecharcode\tempb ]\gl@append ]\to\clist
            \else
               \gl@lop\alist\to\tempa
...
%      \ifgl@loopmore
%         \gl@lop\blist\to\tempb
%         \expandafter\process\tempb\@nil
%         \ifx\alist\empty \ifx\blist\empty \gl@loopmorefalse \fi\fi
%   \repeat
%   \global\let#2=\clist
%}
\newif\ifep@exceptional
\newtoks\mytoks
\def\@showtoks{\@getoptionalarg\@showtoks@i}
\def\@showtoks@i #1{\mytoks=\expandafter{#1}\writeln{%
   \@optionalarg\the\mytoks}}
\def\process #1#2\@nil{%
%{\def\\{!!}\writeln{tempb: \showtoks\tempb, first arg: #1, second arg: #2}}%
   \def\temp{#2}%
   \ifx\temp\empty
      \ep@exceptionaltrue
      \if\ep@samecharcode#1+\gl@append +\to\clist
         \else\if\ep@samecharcode#1@\gl@append @\to\clist
         \else\if\ep@samecharcode#1[\gl@append [\to\clist
         \else\if\ep@samecharcode#1]\gl@append ]\to\clist
         \else \ep@exceptionalfalse
      \fi\fi\fi\fi
   \else
      \ep@exceptionalfalse
      \ifx#1\nogloss \expandafter\gl@append\tempb\to\clist \ep@exceptionaltrue
      \fi
   \fi
   \ifep@exceptional %\gl@append\tempb\to\clist
   \else
      \gl@lop\alist\to\tempa
      \expandafter\gl@append \tempa \to \tempb
      \expandafter\gl@append \tempb \to \clist
...
\def\glw@dodiacritics #1#2\@nil{%
   ...
   \ifx\tempa\empty
      \glw@wordfalse
      \if\ep@samecharcode         #1+\gl@append +\to\mainlist
         \else\if\ep@samecharcode #1@\gl@append @\to\mainlist
         \else\if\ep@samecharcode #1[\gl@append [\to\mainlist
         \else\if\ep@samecharcode #1]\gl@append ]\to\mainlist
         \else \glw@wordtrue
         \fi\fi\fi\fi
   \else
      \glw@wordtrue
      \ifx#1\nogloss \gl@append \nogloss{#2}\to\mainlist \glw@wordfalse\fi
   \fi
}
%
...
%      \if\ep@samecharcode         +\tempa
%            \vskip\epd@gllineskip
%            \@glaparsestate=1
%            \leavevmode
%         \else\if\ep@samecharcode @\tempa
%            \@glaparsestate=1
%         \else\if\ep@samecharcode [\tempa
...
\def\gl@lopTL#1\to#2{\ifx#1\empty
   \let#2\empty \else\gl@lopoffTL#1#2\gl@lopoffTL#1#2\fi}
